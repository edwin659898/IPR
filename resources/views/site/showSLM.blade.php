...
<div class="col-sm-12">
  <div class="card">
    ...
    <div class="card-body">
      <form action="{{route('site.updateSLM',$todos->id)}}" method="post" enctype="multipart/form-data">
        @csrf
        @method('patch')
        ...
        <div class="form-row mt-3">
          <div class="col-md-3 mb-3">
            ...
            <select class="form-control" name="status" required="required">
              ...
              <option value="SLM approved">SLM approved</option>
              <option value="SLM declined">SLM declined</option>
              <option value="SLO approved">SLO approved</option>
              <option value="SLO declined">SLO declined</option>
            </select>
          </div>
          <div class="col-md-3 mb-3">
            <label class="text-green-800 font-bold mr-1">SLM Date Review:</label>
            <input type="date" readonly name="slmD" value="{{ date('Y-m-d') }}" class="form-control" required="required">
          </div>
          <div class="col-md-3 mb-3">
            <label class="text-green-800 font-bold mr-1">SLM Name:</label>
            <input type="text" name="slmN" readonly value="{{auth()->user()->name}}" class="form-control">
          </div>
          <div class="col-md-3 mb-3">
            <label class="text-green-800 font-bold mr-1">SLM Comments:</label>
            <textarea name="slmC" class="form-control">{{$todos->slmC}}</textarea>
          </div>
        </div>
